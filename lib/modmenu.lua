-- source: https://github.com/jaggednz/midigrid
--
...
function ModMenu:doInit()
  ...
  for i, param in ipairs(self.params.params) do
    if param.t == self.params.tBINARY then
      if self.params:lookup_param(i).behavior == 'trigger' then
        -- upstream norns sets this to 2, which causes spurious flash when loading menu
        m.triggered[i] = 0
      else
        m.on[i] = self.params:get(i)
      end
...
local paramset = include '7u-patch-companion/lib/paramset-mod'
